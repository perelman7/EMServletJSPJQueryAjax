...
function delete_dep(){    
    ...
    $.ajax({
        ...
        success : function(data) {
            ...
            $('#resp_d').html("<dialog id=\"dialog\"><p>"+json.message+
                            "</p><button class=\"close_dialog btn btn-outline-dark\">Close</button></dialog>");
            var dialog = document.getElementById('dialog');
            dialog.show();
        },
    ...
    $.ajax({
            ...
            success : function(data) {
                ...
                if(Number(json.status) === 1){
                    ...
                    allDeps[index].departmmentName = name_dep;
...
function save_add_dep(){
    ...
    $.ajax({
            ...
            success : function(data) {
                ...
                console.log(json);
                if(Number(json.status) === 1){
                    allDeps.push({'id': Number(json.idElem), 'departmmentName': name_dep, 'description': desc});
                    createTableD();
                }
                $('#resp_d').html("<dialog id=\"dialog\"><p>"+json.message+
                            "</p><button class=\"close_dialog btn btn-outline-dark\">Close</button></dialog>");
