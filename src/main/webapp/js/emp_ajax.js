...
function delete_emp(){
    ...
    $.ajax({
        ...
        success : function(data) {
            ...
            $('#resp_e').html("<dialog id=\"dialog\"><p>"+json.message+
                            "</p><button class=\"close_dialog btn btn-outline-dark\">Close</button></dialog>");
                var dialog = document.getElementById('dialog');
            dialog.show();
        },
    ...
    $.ajax({
            ...
            success : function(data) {
                ...
                if(Number(json.status) === 1){
                    ...
                    allEmps[index].dataOfBirthday = dataOfBirthday;
...
function save_add_emp(){
    ...
    var dep_id = document.getElementById("departmentName_add").value;
    var dep_name = document.getElementById('departmentName_add').selectedOptions[0].text;
    ...
    $.ajax({
            ...
            data: {
                ...
                fatherName: fatherName,
                dataOfBirthday: dataOfBirthday,
                departmentName: dep_id
            },
            success : function(data) {
                var json = JSON.parse( data );
                if(Number(json.status) === 1){
                    allEmps.push({'id': Number(json.idElem), 'surname': surname, 'name': name, 'fatherName': fatherName,
                        'dataOfBirthday': dataOfBirthday, 'departmentName': dep_name, 'dep_id': dep_id});
                    createTable();
                }
                $('#resp_e').html("<dialog id=\"dialog\"><p>"+json.message+
                                "</p><button class=\"close_dialog btn btn-outline-dark\">Close</button></dialog>");
